...
test('FeedEdit isDisabled = true', () => {
  const render = renderer.create(<FeedEdit isEditing={false} isDisabled isScrolled={false} onAdd={action} onEdit={action} />);
  expect(render).toMatchSnapshot();
});

test('FeedEdit isScrolled = true', () => {
  const render = renderer.create(<FeedEdit isEditing={false} isDisabled={false} isScrolled onAdd={action} onEdit={action} />);
  expect(render).toMatchSnapshot();
});