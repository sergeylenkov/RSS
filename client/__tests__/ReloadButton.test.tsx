...
test('ReloadButton isActive = false', () => {
  const render = renderer.create(<ReloadButton isActive={false} isError={false} isSelected={false} count={10} onClick={action} onUpdate={action} />);
  expect(render).toMatchSnapshot();
});

test('ReloadButton isError = true', () => {
  const render = renderer.create(<ReloadButton isActive={false} isError isSelected={false} count={10} onClick={action} onUpdate={action} />);
  expect(render).toMatchSnapshot();
});
