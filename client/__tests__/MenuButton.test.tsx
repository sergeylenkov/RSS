...
test('MenuButton isSelected = false', () => {
  const render = renderer.create(<MenuButton title={'Test'} isSelected={false} count={10} onClick={action} />);
  expect(render).toMatchSnapshot();
});

test('MenuButton count = 0', () => {
  const render = renderer.create(<MenuButton title={'Test'} isSelected count={0} onClick={action} />);
  expect(render).toMatchSnapshot();
});